  ...
  return <div className="app-layout">
    ...
    <aside id="sidebar" className={`sidebar ${open ? 'open' : ''}`}>
      ...
      <nav aria-label="주 메뉴" onClick={() => setOpen(false)}>
        <section><h2>조사 관리</h2><NavLink end className={link} to="/">대시보드</NavLink><NavLink end className={link} to="/admin/new">새 조사</NavLink><NavLink end className={link} to="/surveys">조사 목록</NavLink><NavLink className={link} to="/surveys/run">조사 진행</NavLink></section>
        <section><h2>기록 및 분석</h2><NavLink end className={link} to="/surveys/completion">완료 현황</NavLink><NavLink end className={link} to="/surveys/search">조사 기록 검색</NavLink><NavLink end className={link} to="/reports">결과 분석</NavLink><NavLink end className={link} to="/reports/print">보고서</NavLink></section>
